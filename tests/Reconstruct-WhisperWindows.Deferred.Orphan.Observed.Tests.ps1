Describe "deferred orphan observed behavior" {
    ...
    It "observes that an un-reanchored deferred prefix word is discarded" {
        $windows = @(
            [PSCustomObject]@{
                Start = 0.0
                End   = 5.0
        ...
        @($words | Where-Object { $_.Text -eq ' Y' }).Count | Should Be 0
