Describe "deferred orphan clean characterization" {
    ...
    It "shows that a deferred prefix word with no later anchor is not retained" {
        $windows = @(
            [PSCustomObject]@{
                Start = 0.0
                End   = 5.0
        ...
        @($words | Where-Object { $_.Text -eq ' X' }).Count | Should Be 0
        ...
        @($words | Where-Object { $_.Text -eq ' D' }).Count | Should Be 1
    }
}
