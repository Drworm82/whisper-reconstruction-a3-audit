...
function Reconstruct-WhisperWindows {
    ...
    for ($i = 1; $i -lt $windows.Count; $i++) {
        ...
        # Temporal placement is a reconstruction concern because only this
        # layer knows the accumulated prefix. A valid MATCH may begin exactly
        # at the prefix boundary; an earlier start would make the replacement
        # overlap already-accepted transcript content.
        if (
            $prefix.Count -gt 0 -and
            [double]$currentMatch[0].From -lt [double]$prefix[-1].To
        ) {
            Write-Host "MATCH REJECTED BY TEMPORAL PREFIX BOUNDARY"
            Write-Host "Prefix boundary: '$($prefix[-1].Text)' @ $($prefix[-1].From)-$($prefix[-1].To)s"
            Write-Host "Current anchor:  '$($currentMatch[0].Text)' @ $($currentMatch[0].From)-$($currentMatch[0].To)s"

            # Route the candidate through the existing SIN MATCH path. This
            # keeps the reconstruction state and deduplication behavior intact.
            $match = $null
        }
        ...
        if (
            ...
            # Use the same SIN MATCH reconstruction path without re-entering
            # the transition from the top-level loop.
            $bandDuration = $overlapEnd - $overlapStart
            $driftAllowance = [math]::Max(
                0.5,
                [math]::Min(1.5, $bandDuration * 0.2)
